...
<script lang="ts">
  ...
  @Component({})
  export default class UpgradeToPremium extends Vue {
    stellarPrice: number = 0;
    isStep1Completed: boolean = false;
    isStep2Completed: boolean = false;
    isStep3Completed: boolean = false;
    depositMemo: string = '';
    depositWallet: string = '';
    numberMonths: number = 1;
    monthlyDiscount: number = 2;
    baseMonthlyPrice: number = 4.99;
    stellarTransaction: string = '';
    isTransactionStatusLoading: boolean = false;
    transactionStatus: string = '';
    created() {
      this.getStellarPrice();
    }
    get discountedMonthlyPrice() {
    ...
    getStellarPrice() {
      axios({
        ...
        url: process.env.VUE_APP_HTTP_URL + "/stellar_price",
      }).then(response => {
    ...
    completeStep1() {
      axios({
        ...
        url: process.env.VUE_APP_HTTP_URL + "/transaction_credentials",
      }).then(response => {
        if (response.status === 200) {
    ...
    checkTransaction() {
      ...
      axios({
        ...
        url: process.env.VUE_APP_HTTP_URL + "/buy_months",
        data: { 
    ...
    copyTextToClipboard(element_id: string) {
